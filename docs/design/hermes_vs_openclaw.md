...
TerminalClaw 当前有两种解释模式，对 Agent 层的需求不同：

### 3.1 API 直连模式（当前默认，推荐）

右面板 Live Monitor 用 Python requests 直调 DeepSeek API。完全不经过 Agent 框架，Hermes vs OpenClaw 对此模式无影响。

### 3.2 OpenClaw Agent 模式（远期选项）

右面板启动一个 Agent 进程，用户可以直接输入自然语言分析指令，Agent 用 read/exec 工具读取日志文件并分析。

**对此模式的需求：**

| 需求 | OpenClaw | Hermes |
|------|---------|--------|
| ① 必须能以 CLI 命令启动，非交互式初始化 | `openclaw agent --session-id terminalclaw -m "system prompt"` ✅ | 无等价命令 ❌ |
| ② 必须能读取文件系统 (read / exec) | `read /tmp/tclaw/live.log` ✅ | 无原生 read 工具 ❌ |
| ③ 必须能在 tmux 面板中作为守护进程常驻 | ✅ 在 tmux 面板中启动，持续等待用户输入 | ❌ TUI 独占终端 |

## 四、Hermes 的 watch_patterns 能力 — 看似相关，实则不同

Hermes v0.9.0 加入了 `watch_patterns` 功能，但关键区别在于：
...
Hermes 的 `watch_patterns` 支持按正则匹配后台进程输出。TerminalClaw 在实现 `TCLAW_TRIGGER_PATTERNS` 时，可以参考 Hermes 的配置语法——支持多个 pattern，每个可独立配置行为。

### 5.2 多 Provider 切换体验

Hermes 的 `hermes model` 命令可以在对话中实时切换模型 Provider，无需重启。TerminalClaw 的 Live Monitor 也需要这种能力。

### 5.3 `hermes claw migrate` 的迁移路径

Hermes 内置了从 OpenClaw 迁移的命令，说明：
- OpenClaw 用户群足够大到有了专用迁移工具
- Hermes 团队认可 OpenClaw 生态中的重要用户群
...
TerminalClaw 需要的是一个"可被子进程调用的 Agent 运行时"，而 Hermes 是一个"面向终端用户的 TUI 对话应用"。两者的交互模型完全不同。
